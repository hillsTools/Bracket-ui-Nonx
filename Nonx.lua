...
local function InitScreen()
	...
	ToggleButton.MouseButton1Click:Connect(function()
		if ScreenAsset.Window then
			ScreenAsset.Window.Visible = not ScreenAsset.Window.Visible
		end
	end)
...
local function InitWindow(ScreenAsset,Window)
	...
	if isMobile then
		-- Increase font sizes
		WindowAsset.Title.TextSize = 18
		
		-- Make resize handle larger for touch
		WindowAsset.Resize.Size = UDim2.new(0, 20, 0, 20)
	end
...
local function InitTab(ScreenAsset,WindowAsset,Window,Tab)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		TabButtonAsset.TextSize = 14
		TabButtonAsset.Size = UDim2.new(0,TabButtonAsset.TextBounds.X + 12,1,-1)
		
		-- Make scrolling frames more touch-friendly
		TabAsset.ScrollBarThickness = 8
		TabAsset.LeftSide.CanvasSize = UDim2.new(0, 0, 0, 0)
		TabAsset.RightSide.CanvasSize = UDim2.new(0, 0, 0, 0)
	end
...
local function InitSection(Parent,Section)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		SectionAsset.Title.TextSize = 16
	end
...
local function InitButton(Parent,ScreenAsset,Window,Button)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		ButtonAsset.Title.TextSize = 16
		ButtonAsset.Size = UDim2.new(1,0,0,30)
	end
...
local function InitToggle(Parent,ScreenAsset,Window,Toggle)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		ToggleAsset.Title.TextSize = 16
		ToggleAsset.Size = UDim2.new(1,0,0,30)
		ToggleAsset.Tick.Size = UDim2.new(0, 20, 0, 20)
	end
...
local function InitSlider(Parent,ScreenAsset,Window,Slider)
	...
	-- Mobile optimizations - make sliders fatter
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		SliderAsset.Title.TextSize = 16
		SliderAsset.Value.TextSize = 14
		SliderAsset.Background.Size = UDim2.new(1, 0, 0, 20)  -- Fatter slider
		SliderAsset.Size = UDim2.new(1, 0, 0, 40)  -- Taller slider container
	end
...
local function InitTextbox(Parent,ScreenAsset,Window,Textbox)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		TextboxAsset.Title.TextSize = 16
		TextboxAsset.Background.Input.TextSize = 14
		TextboxAsset.Size = UDim2.new(1, 0, 0, 50)
	end
...
local function InitKeybind(Parent,ScreenAsset,Window,Keybind)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		KeybindAsset.Title.TextSize = 16
		KeybindAsset.Value.TextSize = 14
		KeybindAsset.Size = UDim2.new(1, 0, 0, 30)
	end
...
local function InitDropdown(Parent,ScreenAsset,Window,Dropdown)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		DropdownAsset.Title.TextSize = 16
		DropdownAsset.Background.Value.TextSize = 14
		DropdownAsset.Size = UDim2.new(1, 0, 0, 40)
	end
	...
	for Index, Option in pairs(Dropdown.List) do
		...
		if isMobile then
			...
			OptionAsset.Size = UDim2.new(1, 0, 0, 30)
		end
...
local function InitColorpicker(Parent,ScreenAsset,Window,Colorpicker)
	...
	local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled
	if isMobile then
		ColorpickerAsset.Title.TextSize = 16
		ColorpickerAsset.Size = UDim2.new(1, 0, 0, 40)
		ColorpickerAsset.Color.Size = UDim2.new(0, 30, 0, 30)
	end
...
function Bracket:Notification(Notification)
	...
	if Notification.Duration then
		task.spawn(function()
			for Time = Notification.Duration,1,-1 then
